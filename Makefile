...
HEADERS += $(wildcard csrc/kimi_k3_decode/*.cuh)
# The decode headers that outgrew a thousand lines are umbrellas over a
# directory of parts, so the rebuild trigger has to reach into each of them.
HEADERS += $(wildcard csrc/kimi_k3_decode/dependency_local/*.cuh)
HEADERS += $(wildcard csrc/kimi_k3_decode/entrypoint/*.cuh)
HEADERS += $(wildcard csrc/kimi_k3_decode/fused_w13/*.cuh)
HEADERS += $(wildcard csrc/kimi_k3_decode/layout/*.cuh)
HEADERS += $(wildcard csrc/kimi_k3_decode/persistent/*.cuh)
NVCC ?= nvcc
ARCH ?= SM103
PYTHON ?= python3
...
# How far to widen every bounded spin's clock budget. One is what production
# ships. Only a compute-sanitizer image raises it, and only because the tool's
# instrumentation can hold a legitimate rendezvous past fifteen seconds -- at
# which point the watchdog's `trap` ends the launch and the tool reports zero
# hazards for a run that never finished. See `csrc/serial_sync.cuh`.
WAIT_TIMEOUT_SCALE ?= 1
...
NVCCFLAGS += -DMOK_WAIT_TIMEOUT_SCALE=$(WAIT_TIMEOUT_SCALE)
